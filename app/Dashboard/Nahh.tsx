...
function Page() {
  ...
  async function getAllVectors(namespace: string) {
    console.log("🚀 [Dashboard] Starting getAllVectors for namespace:", namespace);
    setLoadingCloudData(true);
    console.log("📝 [Dashboard] Setting loading state to true");
    
    // Navigate immediately and set the cloud name
    console.log("🏪 [Dashboard] Setting currentCloudName in store:", namespace);
  ...
  return (
    ...
    //   <CloudPage refreshCloudData={() => getAllVectors(cloudOpen.name)} data={cloudOpen.data} name={cloudOpen.name} onBack={() => setCloudOpen({})} />
    // ):(
