...
User's question: ${inputText}
    ...
    // Search branch
    const namespace = pc.index(indexName, indexHost).namespace(cloudName);
    const response = await namespace.searchRecords({
      query: {
        topK: 4,
        inputs: { text: finalOut.content },
      },
      fields: ['text'],
    });
    ...
    3. Only if the context is insufficient, first state: 
       "The provided context does not fully answer the question, so I am using my own pre-trained knowledge."
